...
//            ->with('absences', $absences);
    ...
    public function createDouala($semestre, $specialite)
    {
        $specialites = $this->specialiteRepository->findWithoutFail($specialite);
        $ec = $specialites->ecues->where('semestre_id', $semestre);
        $cycle = $this->semestreRepository->find($semestre)->cycle;
        $contrats = $this->contratRepository->findWhere(['academic_year_id' => $this->academicYear->id,'cycle_id'=> $cycle->id, 'specialite_id' => $specialites->id, 'ville_id' => 1]);
        ...
        $ens = $this->enseignementRepository->findWhereIn('ecue_id', $ecues)->where('academic_year_id', $this->academicYear->id)->where('specialite_id', $specialite)->where('ville_id', 1);
        foreach ($ens as $enseignement){
            $enseignements[$enseignement->ecue->id] = $enseignement->ecue->title;
        }

        return view('absences.createDouala', compact('enseignements','contrats'));
    }
    ...
    public function afficheDouala($semestre, $specialite){
        ...
        return view('absences.afficheDouala', compact('contrats', 'sem', 'ecues'));
    }
    ...
    public function ficheDouala($semestre, $specialite){
        $specialites = $this->specialiteRepository->findWithoutFail($specialite);
        $ec = $specialites->ecues->where('semestre_id', $semestre);
        ...
        return view('absences.fiche-douala', compact('ec', 'semestre', 'specialite', 'ens'));
    }

    public function ficheEcueDouala($semestre, $specialite, $id){
        $specialites = $this->specialiteRepository->findWithoutFail($specialite);
        $cycle = $this->semestreRepository->find($semestre)->cycle;
        /*
        $contrats = $this->contratRepository->findWhere(['academic_year_id' => $this->academicYear->id,'cycle_id'=> $cycle->id, 'specialite_id' => $specialites->id]);
        */
        ...
        return view('absences.fiche-presence-douala', compact('enseignement', 'apprenants'));
    }
