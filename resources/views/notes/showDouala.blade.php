...
@section('content')
    ...
    <div class="content">
        <div class="box box-primary">
            <div class="box-body">
                {!! Form::open(['url' => 'notes1/'.$type.'/'.$enseignement->id]) !!}
                <table class="table table-responsive table-stripped">
                    <input type="hidden" name="ville_id" value="{{ $ville_id }}">
                    <thead>
                        <tr>
                            <th>Apprenant</th>
                            <th>Note {{ $type }}</th>
                    ...
                    @foreach($contrats as $contrat)
                        ...
                        <tr>
                            ...
                            <td>{{ Form::number($contrat->id, null, ['class' => 'form-control' , 'id' => $contrat->id, 'step' => 'any', 'max' => 20]) }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            ...
            <div class="box-footer text-right m">
                ...
                <a href="{{ route('notes.afficheNotes', [$enseignement->ecue->semestre_id, $enseignement->specialite_id, $ville_id]) }}" class="btn btn-default">
                    Back
                </a>
            </div>
            {!! Form::close() !!}
            ...
            @foreach($contrats as $contrat)
                @if($contrat->notes->where('enseignement_id', $enseignement->id)->first())
                    @if($type == 'cc')
                        ...
                        @if(!$contrat->notes->where('enseignement_id', $enseignement->id)->first()->state_session1 || !$contrat->notes->where('enseignement_id', $enseignement->id)->first()->state_session2)
                    ...
                    @elseif($type == 'session1')
                        ...
                        @if(!$contrat->notes->where('enseignement_id', $enseignement->id)->first()->state_session1)
                            //$("#"+ {!! $contrat->id !!}).prop('disabled', 'true');
                        @endif
                    @else
                        $("#"+ {!! $contrat->id !!}).val({!! $contrat->notes->where('enseignement_id', $enseignement->id)->first()->session2 !!})
                        @if(!$contrat->notes->where('enseignement_id', $enseignement->id)->first()->state_session2)
                            //$("#"+ {!! $contrat->id !!}).prop('disabled', 'true');
                        @endif
                    @endif
                @endif
